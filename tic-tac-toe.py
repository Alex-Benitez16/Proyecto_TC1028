...
# Para esta función si investigué. Importe dos librerias 
...
# Para esta función también tuve que investigar
...
def print_instructions(player1, player2):
    print_cool(f"Alright, alright. {player1}, {player2}, I need to make something very, very clear.")
    print_cool(f"This ain't no regular tic-tac-toe. No, no, no.")
    print_cool(f"Have it known that thi IS WAR.")
    print_cool(f"Make no mistake. Your honor is on the line. Your next 50 years WILL be detirmined by this one game.")
    print_cool(f"Do you understand now?    Good.")
    
    press_enter_to_continue()
    clear_terminal()

    print_cool(f"So, I bet you are wondering why is that. Allow me to explain. ")

    print_cool_fast(f"Let me take you back... back to a time long before we ever thought to use mere symbols on a grid for entertainment.")
    print_cool_fast(f"Centuries ago, in a realm not bound by time or space, there existed two opposing forces: Order and Chaos.")
    print_cool_fast(f"They were not just cosmic ideas, no, they were living, breathing titans. Their battles raged for eons.")
    print_cool_fast(f"And at the heart of it all, their eternal struggle, was a sacred game. A game so powerful, so dangerous, it could shape entire universes.")

    print_cool_fast(f"The game? A simple grid. Nine spaces. But it was far more than that. To mere mortals, it may look like child's play.")
    print_cool_fast(f"But to the Titans? It was the ultimate test of strategy, cunning, and resilience.")
    print_cool_fast(f"Each mark placed on the grid did not just fill a space. It sent shockwaves across galaxies. A move was more than a move.")
    print_cool_fast(f"It was an assertion of will, an imposition of destiny.")

    print_cool_fast(f"For every round of this game, the balance of existence teetered. A single victory could mean prosperity for an entire universe.")
    print_cool_fast(f"A single defeat could mean ruin, as worlds would crumble into dust, swallowed by the void.")
    print_cool_fast(f"Order and Chaos were locked in this conflict, neither side able to claim ultimate victory. Not until the fateful day... when they disappeared.")
    print_cool_fast(f"No one knows how or why. Some say they destroyed each other. Others say they ascended beyond our understanding.")
    print_cool_fast(f"But they left behind their game. And with it, the rules. And with those rules, their power.")

    print_cool_fast(f"The game found its way through the cosmos, passing through the hands of gods, emperors, warriors, and scholars.")
    print_cool_fast(f"Each time it was played, reality itself trembled. Yet, the true nature of the game remained hidden, passed off as mere amusement for the naive.")
    print_cool_fast(f"But you... {player1}, {player2}... you are different.")
    print_cool_fast(f"You have been chosen. Whether by fate, destiny, or some ancient force, you now hold the power of the Titans in your hands.")
    print_cool_fast(f"You may think this is a game of tic-tac-toe, but know this: every move, every decision, ripples through the fabric of reality.")
    
    print_cool_fast(f"Win... and you will shape your world in your image. The stars will align for you, and your name will echo in the halls of eternity.")
    print_cool_fast(f"Lose... and Chaos will consume everything you hold dear. The very ground beneath you will tremble, the skies will darken, and your legacy?")
    ...
    print_cool(f"{player1.upper()} VS {player2.upper()}. LET US SEE WHO SHALL GAIN THE GLORY. ")
